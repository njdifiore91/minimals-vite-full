...
-- These indexes improve query performance for common access patterns and are critical for
-- maintaining performance as the database grows with production usage.
...
-- Improves performance for queries that filter by review status
...
-- Improves performance for queries that join documents with applications
...
-- Index for filtering documents by classification confidence
-- Improves performance for queries that filter by classification confidence
...
-- Improves performance for queries that join merchant details with applications
...
-- Improves performance for queries that filter webhooks by event type
CREATE INDEX idx_webhook_event_type ON webhook(event_type);

-- Index for filtering active webhooks
-- Improves performance for queries that filter for active webhooks only
